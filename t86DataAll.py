...
try:
    for date in datelist:
        ...
        while cnt < 20:
            ...
            logging.debug("    prox server = " +
                          proxList[proxidx] + ", cnt = " + str(proxidx))

            if type == 't86':
                #r = twstock.t86.get('ALL', date, 'json', proxies, logging)
                r = twstock.t86.get('ALL', date, 'json',
                                    proxies, type, logging)
            elif type == 'stockDay':
                #r = twstock.t86.STOCK_DAY('ALLBUT0999', date, 'json', proxies, logging)
                r = twstock.t86.get('ALLBUT0999', date,
                                    'json', proxies, type, logging)
            elif type == 'tpex86':
